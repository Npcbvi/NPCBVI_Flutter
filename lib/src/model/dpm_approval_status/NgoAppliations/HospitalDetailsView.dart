class HospitalDetailsView {
  String message;
  bool status;
  List<DataHospitalDetailsView> data;
  Null list;
  ...
  HospitalDetailsView.fromJson(Map<String, dynamic> json) {
    ...
    status = json['status'];
    if (json['data'] != null) {
      data = <DataHospitalDetailsView>[];
      json['data'].forEach((v) {
        data.add(new DataHospitalDetailsView.fromJson(v));
      });
    }
    list = json['list'];
  }
...
class DataHospitalDetailsView {
  ...
  DataHospitalDetailsView.fromJson(Map<String, dynamic> json) {
    sNo = json['sNo'];
    fax = json['fax'];
