...
class SquintPdfFile extends StatefulWidget {
  final String id;
  final String orgNAme;

  const SquintPdfFile({Key key, this.id,this.orgNAme}) : super(key: key);

  @override
  _SquintPdfFile createState() =>
      _SquintPdfFile();
}

class _SquintPdfFile
    extends State<SquintPdfFile> {
  bool isLoading = true;
  List<SquintPdfFileData> cataractDataList = [];
  ...
  Future<void> fetchCataractDetails() async {
    ...
    try {
      ...
      List<SquintPdfFileData> data =
      await ApiController.getDPM_SquintReportView(
        "string",
        widget.id ?? "",
  ...
  Widget buildDetailCard(SquintPdfFileData data) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Column(
        ...
        children: [
          ...
          keyValue("Deviation Eso/Exo", data.deviationEsoExo),
          keyValue("Squint Type", data.squinttype),
