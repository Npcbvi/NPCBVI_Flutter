...
class CataractPdfViewClick extends StatefulWidget {
  final String id;
  final String orgNAme;

  const CataractPdfViewClick({Key key, this.id,this.orgNAme}) : super(key: key);

  @override
  _CataractPdfViewClick createState() =>
      _CataractPdfViewClick();
}

class _CataractPdfViewClick
    extends State<CataractPdfViewClick> {
  bool isLoading = true;
  ...
  Widget buildDetailCard(ViewClickCatractPdfTypeData data) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Column(
        ...
        children: [
          ...
          keyValue("Registered for", data.registeredfor),
          keyValue("Patient Name", data.name),
          keyValue("Date of Birth", Utils.formatDateString(data.dob)),
          keyValue("Gender", data.gender),
          keyValue("Address", "${data.addressLine1}, ${data.addressLine2}, ${data.addressLine3}"),
          keyValue("Mobile No.", data.mobile.toString()),
          keyValue("Communication Language", data.languagename),

          sectionTitle("Pre Operative"),
          preOperativeTable(
            hospital: data.hName,
            doctor: data.dName,
            visualAcuityLeft: data.visualAcquityLeftVal,
            visualAcuityRight: data.visualAcquityRightVal,
            ocularLeft: data.ocularLeftVal,
            ocularRight: data.ocularRightVal,
          ),

          sectionTitle("Operative Details"),
          operativeDetailsTable(
            operationDate: Utils.formatDateString(data.operatedOn),
            place: data.operatedAt,
            eyeOperated: data.eyeOperatedUpon == "1" ? "Left" : "Right",
            operationType: data.operationTypeVal,
          ),

          sectionTitle("Post Operative"),
          postOperativeTable(
            medication: data.disMedicineVal?.toString() ?? "",
            complications: data.immediateComplicationVal?.toString() ?? "",
            vaLeft: data.disVaLeftVal?.toString() ?? "",
            vaRight: data.disVaRightVal?.toString() ?? "",
          ),


          sectionTitle("Follow Up"),
          followUpTable(
            followUpPlace: data.followUpPlace?.toString() ?? "",
            followUpDate: Utils.formatDateString(data.followUpDate ?? ""),
            vaLeft: data.presentVaLeftVal?.toString() ?? "",
            vaRight: data.presentVaRightVal?.toString() ?? "",
          ),
          sectionTitle("LEFT EYE"),
          eyeTable({
            "Spherical": data.sphericalLeftVal,
            "Cylindrical": data.cylindricalLeftVal,
            "Axis": data.axisLeftVal,
            "VA": data.vaLeftVal
          }),

          sectionTitle("RIGHT EYE"),
          eyeTable({
            "Spherical": data.sphericalRightVal,
            "Cylindrical": data.cylindricalRightVal,
            "Axis": data.axisRight?.toString(),
            "VA": data.vaRightVal
          }),
        ],
      ),
