...
class RetinopathyofPermaturityPdfFiles extends StatefulWidget {
  final String id;
  final String orgNAme;

  const RetinopathyofPermaturityPdfFiles({Key key, this.id,this.orgNAme}) : super(key: key);

  @override
  _RetinopathyofPermaturityPdfFiles createState() =>
      _RetinopathyofPermaturityPdfFiles();
}

class _RetinopathyofPermaturityPdfFiles
    extends State<RetinopathyofPermaturityPdfFiles> {
  bool isLoading = true;
  List<RetinopathyofPermaturityPdfData> cataractDataList = [];
  ...
  Future<void> fetchCataractDetails() async {
    ...
    try {
      ...
      await ApiController.getDPM_RetinopathyPermaturityReportView(
        "string",
        widget.id ?? "",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text(
            'Detailed View(Retinopathy of Permaturity Patient)',
            maxLines: 2,
            style: TextStyle(
              fontSize: 14,
              color: Colors.white,
  ...
  Widget buildDetailCard(RetinopathyofPermaturityPdfData data) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
