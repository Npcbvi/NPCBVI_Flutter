...
class DiaebticRetinopatrhyPdfFile extends StatefulWidget {
  final String id;
  final String orgNAme;

  const DiaebticRetinopatrhyPdfFile({Key key, this.id,this.orgNAme}) : super(key: key);

  @override
  _DiaebticRetinopatrhyPdfFile createState() =>
      _DiaebticRetinopatrhyPdfFile();
}

class _DiaebticRetinopatrhyPdfFile
    extends State<DiaebticRetinopatrhyPdfFile> {
  bool isLoading = true;
  List<DiabeticRatinopathyPdfFileData> cataractDataList = [];
  ...
  Future<void> fetchCataractDetails() async {
    ...
    try {
      ...
      List<DiabeticRatinopathyPdfFileData> data =
      await ApiController.getDPM_DiabeticReportView(
        "string",
        widget.id ?? "",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text(
            'Detailed View(Diabetic Patient)',
            maxLines: 2,
            style: TextStyle(
              fontSize: 14,
              color: Colors.white,
  ...
  Widget buildDetailCard(DiabeticRatinopathyPdfFileData data) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
