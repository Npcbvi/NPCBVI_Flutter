...
class RetinoblastomaPdfFiles extends StatefulWidget {
  final String id;
  final String orgNAme;

  const RetinoblastomaPdfFiles({Key key, this.id,this.orgNAme}) : super(key: key);

  @override
  _RetinoblastomaPdfFiles createState() =>
      _RetinoblastomaPdfFiles();
}

class _RetinoblastomaPdfFiles
    extends State<RetinoblastomaPdfFiles> {
  bool isLoading = true;
  List<RetinoBlastamPdfFilesData> cataractDataList = [];
  ...
  Future<void> fetchCataractDetails() async {
    ...
    try {
      ...
      List<RetinoBlastamPdfFilesData> data =
      await ApiController.getDPM_RetinoblastomaReportView(
        "string",
        widget.id ?? "",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text(
            'Detailed View(Retinoblastoma Patient)',
            maxLines: 2,
            style: TextStyle(
              fontSize: 14,
              color: Colors.white,
  ...
  Widget buildDetailCard(RetinoBlastamPdfFilesData data) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
