...
class CongentialPdfFile extends StatefulWidget {
  final String id;
  final String orgNAme;

  const CongentialPdfFile({Key key, this.id,this.orgNAme}) : super(key: key);

  @override
  _CongentialPdfFile createState() =>
      _CongentialPdfFile();
}

class _CongentialPdfFile
    extends State<CongentialPdfFile> {
  bool isLoading = true;
  List<CongentialPdfFileData> cataractDataList = [];
  ...
  Future<void> fetchCataractDetails() async {
    ...
    try {
      ...
      List<CongentialPdfFileData> data =
      await ApiController.getDPM_CongenitalReportView(
        "string",
        widget.id ?? "",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text(
            'Detailed View(Congenital Ptosis Patient)',
            maxLines: 2,
            style: TextStyle(
              fontSize: 14,
              color: Colors.white,
  ...
  Widget buildDetailCard(CongentialPdfFileData data) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
