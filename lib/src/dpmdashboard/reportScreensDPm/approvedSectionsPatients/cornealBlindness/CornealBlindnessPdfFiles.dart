...
class CornealBlindnessPdfFiles extends StatefulWidget {
  final String id;
  final String orgNAme;

  const CornealBlindnessPdfFiles({Key key, this.id,this.orgNAme}) : super(key: key);

  @override
  _CornealBlindnessPdfFiles createState() =>
      _CornealBlindnessPdfFiles();
}

class _CornealBlindnessPdfFiles
    extends State<CornealBlindnessPdfFiles> {
  bool isLoading = true;
  List<CornealBlindnessPdfFileData> cataractDataList = [];
  ...
  Future<void> fetchCataractDetails() async {
    ...
    try {
      ...
      List<CornealBlindnessPdfFileData> data =
      await ApiController.getDPM_CornealBlindnessReportView(
        "string",
        widget.id ?? "",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text(
            'Detailed View(Corneal Blindness Patient)',
            maxLines: 2,
            style: TextStyle(
              fontSize: 14,
              color: Colors.white,
  ...
  Widget buildDetailCard(CornealBlindnessPdfFileData data) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
