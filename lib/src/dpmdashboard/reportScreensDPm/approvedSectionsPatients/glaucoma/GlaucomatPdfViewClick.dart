...
import 'package:mohfw_npcbvi/src/model/dpmRegistration/viewClickReportData/galucoma/GlaucomPDfFile.dart';
import 'package:mohfw_npcbvi/src/utils/AppConstants.dart';
import 'package:mohfw_npcbvi/src/utils/Utils.dart';
import 'package:dio/dio.dart';

class GlaucomatPdfViewClick extends StatefulWidget {
  final String id;
  final String orgNAme;

  const GlaucomatPdfViewClick({Key key, this.id,this.orgNAme}) : super(key: key);

  @override
  _GlaucomatPdfViewClick createState() =>
      _GlaucomatPdfViewClick();
}

class _GlaucomatPdfViewClick
    extends State<GlaucomatPdfViewClick> {
  bool isLoading = true;
  List<GlaucomPDfFileData> cataractDataList = [];
  ...
  Future<void> fetchCataractDetails() async {
    ...
    try {
      ...
      List<GlaucomPDfFileData> data =
      await ApiController.getDPM_GlaucomaReportView(
        "string",
        widget.id ?? "",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text(
            'Detailed View (Glaucoma  Patient)',
            maxLines: 2,
            style: TextStyle(
              fontSize: 14,
              color: Colors.white,
  ...
  Widget buildDetailCard(GlaucomPDfFileData data) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Column(
        ...
        //  keyValue("Registered for", data.registeredfor),
          ...
          keyValue('Gender:', mapGender(data.gender)),
          ...
          keyValue("Pin Code", data.pincode.toString()),
          keyValue("Communication Language", data.languagename),
          keyValue("Hospital", data.hName),
          keyValue("Reporting Place", data.reportingplace),
          keyValue("Medicfication on Discharge", data.glaucomaMedicationVal),
          keyValue("Glaucoma Medication", data.glaucomaMedication),
        ],
      ),
    );
  }
